...
body: >
  ...
  Una función puede declarar un tipo de retorno, un nombre, una lista de
  ...
  Aunque este código es válido, los parámetros sin anotación se tratan como
  `dynamic`, lo que reduce la ayuda del analizador estático. Por claridad y
  seguridad, es recomendable declarar los tipos, especialmente en APIs públicas.


  ### Funciones flecha


  Cuando una función contiene una sola expresión, puede escribirse con la
  sintaxis flecha (`=>`):
  ...
  > Después de `=>` solo puede existir una expresión, no un bloque de
  instrucciones.
  ...
  Los parámetros pueden ser posicionales o nombrados. Además, pueden ser
  obligatorios, opcionales o tener valores predeterminados.


  #### Parámetros posicionales obligatorios


  Se escriben directamente entre paréntesis y deben enviarse en el orden
  definido:
  ...
  Se escriben entre corchetes (`[]`). Si no tienen un valor predeterminado,
  deben aceptar `null`:
  ...
  Se escriben entre llaves (`{}`). Al invocar la función, se indica el nombre de
  cada argumento:
  ...
  Una función puede asignarse a una variable, pasarse como argumento o
  ...
  Las funciones anónimas, también llamadas _lambdas_ o _closures_, no tienen
  nombre y suelen utilizarse como argumentos de otras funciones:
  ...
  Una función recursiva se invoca a sí misma hasta alcanzar una condición de
  salida:
  ...
  Las funciones asíncronas utilizan `async` y normalmente devuelven un
  `Future<T>`. La palabra clave `await` permite esperar el resultado de una
  operación asíncrona:
  ...
  Un tipo de función describe los parámetros que recibe una función y el valor
  que devuelve:
  ...
  Si el mismo tipo se utiliza varias veces, puede declararse un alias con
  `typedef`:
  ...
  - Usa nombres que describan la acción realizada, como `obtenerUsuario` o
  `calcularTotal`.

  - Mantén cada función enfocada en una sola responsabilidad.

  - Prefiere funciones flecha únicamente para expresiones cortas y fáciles de
  leer.
  ...
  - [Programación asíncrona: `Future`, `async` y
