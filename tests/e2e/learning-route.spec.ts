...
test('opens the first Flutter note and preserves route order', async ({ page }) => {
  ...
  await expect(navigation.locator('a.route-link')).toHaveCount(3);
  await expect(navigation.locator('a.route-link').first()).toHaveAttribute('aria-current', 'page');
  await expect(navigation.locator('a.route-link').nth(1)).toContainText('Composición de widgets');
});
...
test('shows a friendly empty route', async ({ page }) => {
  await page.goto('/categorias/sdd/');
